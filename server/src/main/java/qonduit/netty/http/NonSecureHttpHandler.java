...
public class NonSecureHttpHandler extends ChannelInboundHandlerAdapter implements HttpHandler {
    ...
    public NonSecureHttpHandler(Configuration conf) {
        String host = conf.getHttp().getHost();
        int port = conf.getHttp().getPort();
        String path = conf.getHttp().getRedirectPath();
        redirectAddress = "https://" + host + ":" + port + path;
    }
